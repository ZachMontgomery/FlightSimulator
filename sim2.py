...
class vehicle:
    ...
    def calc_gain(self, update=False, updateAS=False):
        
        if not update:
            self.text('Enter in values for Autopilot')
            self.VD = float(input('Enter desired value for airspeed (ft/sec): '))
            c = '?'
            while c not in ('a', 'c'):
                c = input('Control altitude or climb angle (a/c): ')
            if c == 'a':
            ...
            c = '?'
            while c not in ('b', 'h'):
                c = input('Control heading or bank angle (h/b): ')
            if c == 'h':
                self.head_flag = True
                self.psi0 = float(input('Enter desired value for heading (deg): ')) * np.pi / 180.
            else:
                self.bank = float(input('Enter desired value for bank (deg): ')) * np.pi / 180.
        ...
        # else:
            # self.alt_flag = False
        if self.alt_flag and abs(-self.states[8] - self.Altitude) < self.altLimit:
            self.climb = 0.
            alt_flag = True
        else:
            alt_flag = False
            if self.alt_flag:
                if self.Altitude > -self.states[8]:
                    self.climb = 15.*np.pi/180.
                else:
                    self.climb = -10.*np.pi/180.
        ...
        # else:
            # self.head_flag = False
        # self.bank = self.temp
        psiTemp = quat.Quat2Euler(self.states[9:13])[-1]
        
        # input('psiTemp in rad {} and deg {}'.format(psiTemp,psiTemp*180/np.pi))
        
        #check for left turn which should be right
        while (psiTemp - self.psi0)*180./np.pi > 180.:
            self.psi0 += 2.*np.pi
        #check for right turn which should be left
        while (self.psi0 - psiTemp)*180./np.pi > 180.:
            self.psi0 -= 2.*np.pi
        
        if self.head_flag and abs(psiTemp - self.psi0)*180./np.pi < self.headLimit:
            head_flag = True
            self.bank = 0.
        else:
            head_flag = False
            if self.head_flag:
                if psiTemp > self.psi0:
                    self.bank = -55.*np.pi/180.
                else:
                    self.bank = 55.*np.pi/180.
        
        airspeedFlag = True
        # if updateAS:
            # V = (self.states[0]**2 + self.states[1]**2 + self.states[2]**2)**0.5
            # if abs(self.VD - V) < 5.:
                # self.V0 = self.VD
                # airspeedFlag = True
            # else:
                # if self.VD > V:
                    # self.V0 = V + 1.01*self.asLimit
                # else:
                    # self.V0 = V - 1.01*self.asLimit
        self.V0 = self.VD
        ...
        if alt_flag:
            z_err = 0.5
            theta_err = 0.1*np.pi/180.
        else:
            z_err = 10000.
            theta_err = 0.05*np.pi/180.
        
        
        
        if head_flag:
            phi_err = 5. * np.pi / 180.
            psi_err = .05 * np.pi / 180.
        else:
            phi_err = .1*np.pi/180.
            psi_err = 1000.
        ...
        return alt_flag, head_flag, airspeedFlag
